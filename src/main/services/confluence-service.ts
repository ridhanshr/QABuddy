import type {
  ConfluenceConfig,
  ExtractedTestCase,
  ExtractionDepth,
  ParseConfluenceEntriesOptions,
  ParseConfluenceParseDebugChunk,
  ParseConfluenceParseDebugReport,
  ParseConfluenceParseDebugRow,
  ParseConfluenceParseDebugTable,
} from "@shared/types";
import { sanitizeExtractedTestCase } from "@shared/types";
import { ConfluenceClient } from "./confluence/confluence-client";
import {
  generateXhtmlTable,
  generateSingleTable,
  generateTocMacro,
  generateSectionHeading,
  normalizeAttachmentOrder,
} from "./confluence/table-formatter";
import { stripHtml, parseConfluenceDisplayUrl, fallbackTestCases, chunkContent, deduplicateTestCases, slugify } from "./utils";
import { logger } from "./logger";
import type { OllamaService } from "./ollama-service";

function normalizeFilename(name: string): string {
  if (!name) return "";
  let decoded = name;
  
  decoded = decoded
    .replace(/&amp;/g, "&")
    .replace(/&quot;/g, '"')
    .replace(/&lt;/g, "<")
    .replace(/&gt;/g, ">")
    .replace(/&#39;/g, "'")
    .replace(/&#x27;/g, "'")
    .replace(/&nbsp;/g, " ");

  try {
    decoded = decodeURIComponent(decoded);
  } catch (e) {
    // ignore
  }

  return decoded.trim().toLowerCase();
}

function decodeHtmlEntities(value: string): string {
  return value
    .replace(/&quot;/g, '"')
    .replace(/&#34;/g, '"')
    .replace(/&apos;/g, "'")
    .replace(/&#39;/g, "'")
    .replace(/&#x27;/g, "'")
    .replace(/&lt;/g, "<")
    .replace(/&gt;/g, ">")
    .replace(/&amp;/g, "&");
}

function detectListFormat(html: string): "plain" | "ordered" | "unordered" {
  const normalized = decodeHtmlEntities(html);
  if (/<ol\b/i.test(normalized)) return "ordered";
  if (/<ul\b/i.test(normalized)) return "unordered";
  return "plain";
}

function isScreenCaptureExpandTitle(title: string): boolean {
  return title === "Screen Capture" || title === "Click here to expand...";
}

function extractAttachmentRefs(block: string): Array<{ name: string; index: number; end: number }> {
  const patterns = [
    /<[^>]*\bri:filename=["']([^"']+)["'][^>]*>/gi,
    /<[^>]*\bdata-linked-resource-default-alias=["']([^"']+)["'][^>]*>/gi,
    /<[^>]*\bdata-file-src=["'][^"']*\/([^"'\/?]+)(?:\?[^"']*)?["'][^>]*>/gi,
    /<[^>]*\bdata-qa-attachment-name=["']([^"']+)["'][^>]*>/gi,
  ];

  for (const pattern of patterns) {
    const refs: Array<{ name: string; index: number; end: number }> = [];
    let match;
    while ((match = pattern.exec(block)) !== null) {
      refs.push({
        name: decodeHtmlEntities(match[1]),
        index: match.index,
        end: match.index + match[0].length,
      });
    }
    if (refs.length > 0) {
      refs.sort((a, b) => a.index - b.index);
      return refs;
    }
  }

  return [];
}

function latestAttachmentGroupBefore(html: string, index: number): string {
  const before = html.slice(0, index);
  const titleRegex =
    /<span\b[^>]*class=["'][^"']*\bexpand-control-text\b[^"']*["'][^>]*>([\s\S]*?)<\/span>|<ac:parameter\b[^>]*ac:name=["']title["'][^>]*>([\s\S]*?)<\/ac:parameter>/gi;
  let group = "";
  let match;
  while ((match = titleRegex.exec(before)) !== null) {
    const title = stripHtml(match[1] || match[2] || "").trim();
    if (title && !isScreenCaptureExpandTitle(title)) {
      group = title;
    }
  }
  return group;
}

function stripAttachmentMarkup(block: string): string {
  return block
    .replace(/<ac:image\b[\s\S]*?<\/ac:image>/gi, "")
    .replace(/<ac:link\b[\s\S]*?<\/ac:link>/gi, "")
    .replace(/<span\b[^>]*class=["'][^"']*\bconfluence-embedded-file-wrapper\b[^"']*["'][^>]*>[\s\S]*?<\/span>/gi, "")
    .replace(/<img\b[^>]*(?:data-linked-resource-default-alias|data-image-src|data-file-src)[^>]*>/gi, "")
    .replace(/<a\b[^>]*data-file-src=["'][^"']+["'][^>]*>[\s\S]*?<\/a>/gi, "");
}

export class ConfluenceService {
  private readonly config: ConfluenceConfig;
  private readonly rawClient: ConfluenceClient;

  constructor(config: ConfluenceConfig) {
    this.config = config;
    this.rawClient = new ConfluenceClient(config);
  }

  async validateConnection(): Promise<string> {
    return this.rawClient.validateConnection();
  }

  async getPage(pageId: string) {
    return this.rawClient.getPage(pageId);
  }

  async getPagePreview(pageId: string): Promise<{ title: string; content: string; version: number }> {
    const page = await this.getPage(pageId);
    return {
      title: page.title,
      content: page.body?.storage?.value || "",
      version: page.version?.number || 1,
    };
  }

  async uploadAttachment(pageId: string, filename: string, base64Data: string) {
    return this.rawClient.uploadAttachment(pageId, filename, base64Data);
  }

  async updatePage(pageId: string, title: string, content: string, version: number) {
    return this.rawClient.updatePage(pageId, title, content, version);
  }

  generateXhtmlTable(entries: any[], jiraBaseUrl?: string, jiraServerId?: string) {
    return generateXhtmlTable(entries, jiraBaseUrl, jiraServerId);
  }

  private stripGeneratedMarkers(content: string): string {
    return content
      .replace(/\s*<p>\s*Generated by QA Buddy\s*[-—]\s*[^<]+<\/p>\s*/gi, "\n")
      .replace(/<div\b[^>]*\bdata-macro-name\s*=\s*"toc"[^>]*>[\s\S]*?<\/div>\s*/gi, "")
      .replace(/<ac:structured-macro\b[^>]*\bac:name\s*=\s*"toc"[^>]*>[\s\S]*?<\/ac:structured-macro>\s*/gi, "")
      .replace(/<h1\b[^>]*>[\s\S]*?<\/h1>\s*/gi, "");
  }

  private normalizeTableSpacing(content: string): string {
    return content
      .replace(/(?:\s*<p>\s*(?:<br\s*\/?>|\u00a0|&nbsp;)?\s*<\/p>\s*){2,}/gi, "\n<p><br /></p>\n")
      .replace(/(<\/table>)\s*(?:<p>\s*(?:<br\s*\/?>|\u00a0|&nbsp;)?\s*<\/p>\s*)+(?=<table\b)/gi, "$1\n<p><br /></p>\n")
      .replace(/^\s+|\s+$/g, "");
  }

  private extractBalancedTagBlock(content: string, startIndex: number, tagName: string): { html: string; end: number } | null {
    const openTag = new RegExp(`<${tagName}\\b`, "i");
    if (!openTag.test(content.slice(startIndex))) {
      return null;
    }

    let depth = 0;
    const tokenRegex = new RegExp(`<\\/?${tagName}\\b[^>]*>`, "gi");
    tokenRegex.lastIndex = startIndex;

    let match: RegExpExecArray | null;
    while ((match = tokenRegex.exec(content)) !== null) {
      const token = match[0];
      if (/^<\//.test(token)) {
        depth--;
        if (depth === 0) {
          const end = tokenRegex.lastIndex;
          return { html: content.slice(startIndex, end), end };
        }
      } else {
        depth++;
      }
    }

    return null;
  }

  private collectParsedTables(content: string): Array<{ qaIndex: number; html: string; start: number; end: number; entry: any; section: string }> {
    const tables: Array<{ qaIndex: number; html: string; start: number; end: number; entry: any; section: string }> = [];
    let currentSection = "";
    const regex = /<h1\b[^>]*>|<table\b[^>]*>/gi;
    let match: RegExpExecArray | null;
    let qaIndex = 0;

    while ((match = regex.exec(content)) !== null) {
      const chunk = match[0];
      if (chunk.startsWith("<h1")) {
        const endMatch = content.slice(regex.lastIndex).match(/<\/h1\s*>/i);
        if (!endMatch || endMatch.index === undefined) {
          regex.lastIndex = match.index + chunk.length;
          continue;
        }
        const start = match.index;
        const end = regex.lastIndex + endMatch.index + endMatch[0].length;
        const fullChunk = content.slice(start, end);
        regex.lastIndex = end;
        const text = fullChunk.replace(/<[^>]+>/g, "").trim();
        if (text) currentSection = text;
      } else {
        const tableBlock = this.extractBalancedTagBlock(content, match.index, "table");
        if (!tableBlock) {
          regex.lastIndex = match.index + chunk.length;
          continue;
        }
        regex.lastIndex = tableBlock.end;
        const parsed = this.parseSingleTable(tableBlock.html, false);
        if (parsed.entry && parsed.entry.testCaseNo) {
          tables.push({
            qaIndex,
            html: tableBlock.html,
            start: match.index,
            end: tableBlock.end,
            entry: parsed.entry,
            section: currentSection,
          });
          qaIndex++;
        }
      }
    }

    return tables;
  }

  private mergeEntriesIntoContent(
    content: string,
    entries: any[],
    jiraBaseUrl?: string,
    jiraServerId?: string,
    deletedTableIndices: number[] = []
  ): { content: string; replacedCount: number; appendedCount: number; deletedCount: number } {
    const parsedTables = this.collectParsedTables(content);
    const deletedSet = new Set(deletedTableIndices);

    const replacementsByIndex = new Map<number, any>();
    const appendedEntries: any[] = [];
    for (const entry of entries) {
      if (typeof entry.sourceTableIndex === "number") {
        replacementsByIndex.set(entry.sourceTableIndex, entry);
      } else {
        appendedEntries.push(entry);
      }
    }

    logger.debug("mergeEntriesIntoContent", `entries=${entries.length} parsedTables=${parsedTables.length} replacements=${replacementsByIndex.size} appended=${appendedEntries.length} deleted=${deletedSet.size}`);

    if (replacementsByIndex.size === 0 && appendedEntries.length === 0 && deletedSet.size === 0) {
      return { content: this.normalizeTableSpacing(this.stripGeneratedMarkers(content)), replacedCount: 0, appendedCount: 0, deletedCount: 0 };
    }

    const outputBySection = new Map<string, Array<{ html: string }>>();

    for (const table of parsedTables) {
      if (deletedSet.has(table.qaIndex)) continue;
      const replacement = replacementsByIndex.get(table.qaIndex);
      if (replacement) {
        const section = replacement.section || "";
        if (!outputBySection.has(section)) outputBySection.set(section, []);
        outputBySection.get(section)!.push({ html: generateSingleTable(replacement, jiraBaseUrl, jiraServerId) });
      } else {
        const section = table.section || "";
        if (!outputBySection.has(section)) outputBySection.set(section, []);
        outputBySection.get(section)!.push({ html: table.html });
      }
    }

    for (const entry of appendedEntries) {
      const section = entry.section || "";
      if (!outputBySection.has(section)) outputBySection.set(section, []);
      outputBySection.get(section)!.push({ html: generateSingleTable(entry, jiraBaseUrl, jiraServerId) });
    }

    let headContent = "";
    let tailContent = "";
    if (parsedTables.length > 0) {
      const firstTableStart = parsedTables[0].start;
      const lastTableEnd = parsedTables[parsedTables.length - 1].end;
      headContent = this.normalizeTableSpacing(this.stripGeneratedMarkers(content.slice(0, firstTableStart))).trim();
      tailContent = this.normalizeTableSpacing(this.stripGeneratedMarkers(content.slice(lastTableEnd))).trim();
    }

    const sectionNames = [...outputBySection.keys()].filter(Boolean);
    const hasSections = sectionNames.length > 0;
    const orderedSections: string[] = hasSections
      ? [...new Set([...sectionNames, ...[...outputBySection.keys()].filter(s => !sectionNames.includes(s))])]
      : [""];

    logger.debug("mergeEntriesIntoContent", `outputBySection=${JSON.stringify([...outputBySection.keys()])} sectionNames=${JSON.stringify(sectionNames)} orderedSections=${JSON.stringify(orderedSections)} hasSections=${hasSections}`);

    let bodyParts: string[] = [];
    if (hasSections) {
      bodyParts.push(generateTocMacro(sectionNames));
    }
    for (const section of orderedSections) {
      const items = outputBySection.get(section) || [];
      if (items.length === 0) continue;
      if (section && hasSections) {
        bodyParts.push(generateSectionHeading(section));
      }
      for (const item of items) {
        bodyParts.push(item.html);
      }
    }

    let result = [headContent, ...bodyParts, tailContent].filter(Boolean).join("\n<p><br /></p>\n");
    result = this.normalizeTableSpacing(result);

    let replacedCount = 0;
    let deletedCount = 0;
    for (const table of parsedTables) {
      if (deletedSet.has(table.qaIndex)) {
        deletedCount++;
      } else if (replacementsByIndex.has(table.qaIndex)) {
        replacedCount++;
      }
    }

    return {
      content: result,
      replacedCount,
      appendedCount: appendedEntries.length,
      deletedCount,
    };
  }

  async syncToConfluence(
    pageId: string,
    entries: any[],
    jiraBaseUrl?: string,
    jiraServerId?: string,
    deletedTableIndices: number[] = []
  ): Promise<{ pageTitle: string; pageUrl: string; entryCount: number; imageCount: number; attachmentCount: number }> {
    const page = await this.getPage(pageId);
    const currentVersion = page.version?.number || 1;
    const content = page.body?.storage?.value || "";

    const finalJiraServerId = jiraServerId || await this.resolveJiraServerId(content);

    let imageCount = 0;
    let attachmentCount = 0;
    for (const entry of entries) {
      const entryImgs = entry.images || [];
      logger.info("Confluence", `Syncing entry "${entry.testCaseNo || entry.scenario || "?"}" with ${entryImgs.length} attachment(s)`);
      for (const img of normalizeAttachmentOrder(entry.images || [])) {
        if (!img.data) {
          logger.info("Confluence", `Skipping attachment "${img.name}" — data kosong`);
          continue;
        }

        const ext = img.name.split(".").pop() || "png";
        const tcNo = (entry.testCaseNo || "TC-000").replace(/\s+/g, "");
        const scenarioSlug = slugify(entry.scenario || "untitled");
        const newName = `${tcNo}-${scenarioSlug}-${img.order}.${ext}`;

        try {
          await this.uploadAttachment(pageId, newName, img.data);
          const originalImg = (entry.images || []).find((i: any) => i.order === img.order);
          if (originalImg) originalImg.name = newName;
          if (img.data.startsWith("data:image/")) {
            imageCount++;
          } else {
            attachmentCount++;
          }
        } catch (uploadErr) {
          const axiosErr = uploadErr as any;
          const status = axiosErr?.response?.status;
          const statusText = axiosErr?.response?.statusText;
          const respData = axiosErr?.response?.data;
          const detail = respData
            ? typeof respData === "string" ? respData.slice(0, 500) : JSON.stringify(respData).slice(0, 500)
            : "no response body";
          logger.error("Confluence", `Failed to upload attachment "${newName}" (HTTP ${status || "?"} ${statusText || ""}): ${detail}`, uploadErr);
        }
      }
    }

    const { content: bodyHtml, replacedCount, appendedCount, deletedCount } = this.mergeEntriesIntoContent(
      content,
      entries,
      jiraBaseUrl,
      finalJiraServerId,
      deletedTableIndices
    );

    const updated = await this.updatePage(pageId, page.title, bodyHtml, currentVersion);

    return {
      pageTitle: updated.title,
      pageUrl: `${this.config.baseUrl.replace(/\/+$/, "")}/spaces/${page.space?.key || ""}/pages/${pageId}`,
      entryCount: replacedCount + appendedCount + deletedCount,
      imageCount,
      attachmentCount,
    };
  }

  async previewSyncXhtml(
    pageId: string,
    entries: any[],
    jiraBaseUrl?: string,
    jiraServerId?: string
  ): Promise<{ currentTitle: string; currentVersion: number; generatedTables: string; entryCount: number; existingEntryCount: number }> {
    const page = await this.getPage(pageId);
    const currentVersion = page.version?.number || 1;
    const content = page.body?.storage?.value || "";

    const existingEntries = this.parseEntriesFromContent(content);
    const finalJiraServerId = jiraServerId || await this.resolveJiraServerId(content);
    const generatedTables = this.generateXhtmlTable(entries, jiraBaseUrl, finalJiraServerId);

    return {
      currentTitle: page.title,
      currentVersion,
      generatedTables,
      entryCount: entries.length,
      existingEntryCount: existingEntries.length,
    };
  }

  async getPageByUrl(url: string) {
    const pageIdMatch = url.match(/pages\/(\d+)/) || url.match(/pageId=(\d+)/);
    const pageId = pageIdMatch ? pageIdMatch[1] : "";
    if (pageId) {
      return this.getPage(pageId);
    }

    const displayInfo = parseConfluenceDisplayUrl(url);
    if (displayInfo) {
      return this.rawClient.getPageByTitle(displayInfo.spaceKey, displayInfo.title);
    }

    throw new Error("Invalid Confluence URL. Could not find page ID or friendly path.");
  }

  async summarize(prompt: string, ollama?: any) {
    return {
      answer: "Fitur pencarian global Confluence belum diimplementasi. Mohon berikan URL spesifik halaman Confluence.",
      pages: []
    };
  }

  async getAttachments(pageId: string): Promise<any[]> {
    return this.rawClient.getAttachments(pageId);
  }

  async downloadAttachment(downloadUrl: string): Promise<Buffer> {
    return this.rawClient.downloadAttachment(downloadUrl);
  }

  async extractTestCases(url: string, depth: string, ollama?: OllamaService, ragContext?: string, ocrText?: string, pageOverride?: any) {
    const page = pageOverride || await this.getPageByUrl(url);
    const content = page.body?.storage?.value || "";
    
    const plainText = stripHtml(content);
    const needsChunking = plainText.length > 12000; // sync with CHUNK_SIZE in utils.ts

    let testCases: ExtractedTestCase[] = [];
    let isFallback = false;

    if (ollama) {
      try {
        if (needsChunking) {
          // ponies: chunked extraction for large content
          const chunks = chunkContent(plainText);
          logger.info("Confluence", `Content chunked into ${chunks.length} parts for extraction`);
          
          const allExtracted: ExtractedTestCase[] = [];
          for (let i = 0; i < chunks.length; i++) {
            logger.info("Confluence", `Extracting part ${i + 1}/${chunks.length}...`);
            let extracted: ExtractedTestCase[] | null = null;
            const chunkInfo = { index: i, total: chunks.length };

            if (ragContext && typeof ollama.extractTestCasesRagEnriched === "function") {
              extracted = await ollama.extractTestCasesRagEnriched(chunks[i], ocrText, ragContext, depth as ExtractionDepth, chunkInfo);
            } else if (ocrText && typeof ollama.extractTestCasesOcrGrounded === "function") {
              extracted = await ollama.extractTestCasesOcrGrounded(chunks[i], ocrText, depth as ExtractionDepth, ragContext, chunkInfo);
            } else if (typeof ollama.extractTestCases === "function") {
              extracted = await ollama.extractTestCases(chunks[i], depth as ExtractionDepth, ragContext, ocrText, chunkInfo);
            }
            
            if (extracted && extracted.length > 0) {
              const sanitized = extracted.map(sanitizeExtractedTestCase).filter((tc): tc is ExtractedTestCase => tc !== null);
              allExtracted.push(...sanitized);
            }
          }
          
          // Deduplicate across chunks
          if (allExtracted.length > 0) {
            testCases = deduplicateTestCases(allExtracted);
            logger.info("Confluence", `Extracted ${allExtracted.length} test cases, ${testCases.length} after deduplication`);
          }
        } else {
          // Standard extraction for small content
          let extracted: ExtractedTestCase[] | null = null;
          if (ragContext && typeof ollama.extractTestCasesRagEnriched === "function") {
            extracted = await ollama.extractTestCasesRagEnriched(plainText, ocrText, ragContext, depth as ExtractionDepth);
          } else if (ocrText && typeof ollama.extractTestCasesOcrGrounded === "function") {
            extracted = await ollama.extractTestCasesOcrGrounded(plainText, ocrText, depth as ExtractionDepth, ragContext);
          } else if (typeof ollama.extractTestCases === "function") {
            extracted = await ollama.extractTestCases(plainText, depth as ExtractionDepth, ragContext, ocrText);
          }

          if (extracted && extracted.length > 0) {
            const sanitized = extracted.map(sanitizeExtractedTestCase).filter((tc): tc is ExtractedTestCase => tc !== null);
            if (sanitized.length > 0) {
              testCases = sanitized;
            }
          }
        }
      } catch (err) {
        logger.error("Confluence", "Ollama extraction failed, falling back to rule-based...", err);
      }
    }

    if (testCases.length === 0) {
      testCases = fallbackTestCases(stripHtml(content), depth);
      isFallback = true;
    }
    
    return {
      testCases,
      pageTitle: page.title,
      sourceUrl: url,
      isFallback
    };
  }

  private async resolveJiraServerId(content: string): Promise<string | undefined> {
    const fromContent = this.detectJiraServerId(content);
    if (fromContent) {
      return fromContent;
    }
    return this.rawClient.detectJiraServerIdFromAppLinks();
  }

  detectJiraServerId(content: string): string | undefined {
    const normalized = decodeHtmlEntities(content);
    const patterns = [
      /<ac:parameter\b[^>]*ac:name=["']serverId["'][^>]*>\s*([^<]+?)\s*<\/ac:parameter>/i,
      /<ri:server\b[^>]*ri:server-id=["']([^"']+)["'][^>]*\/?>/i,
      /\bdata-jira-server-id=["']([^"']+)["']/i,
      /\bdata-server-id=["']([^"']+)["']/i,
      /\bserver-id=["']([^"']+)["']/i,
      /["']serverId["']\s*[:=]\s*["']([^"']+)["']/i,
      /\bserverId=["']([^"']+)["']/i,
      /\bserverId=([A-Za-z0-9:._-]+)/i,
    ];

    for (const pattern of patterns) {
      const match = normalized.match(pattern);
      if (match?.[1]?.trim()) {
        return match[1].trim();
      }
    }

    return undefined;
  }

  parseEntriesFromContent(content: string): any[] {
    return this.collectParsedTables(content).map((table) => ({
      ...table.entry,
      section: table.section || "",
      sourceTableIndex: table.qaIndex,
      sourceTableHtml: table.html,
      isDirty: false,
    }));
  }

  async parseEntriesFromPage(pageId: string, options?: ParseConfluenceEntriesOptions): Promise<{
    pageId: string;
    pageTitle: string;
    contentLoaded: boolean;
    entries: any[];
    jiraServerId?: string;
    error?: string;
    debug?: ParseConfluenceParseDebugReport;
  }> {
    try {
      const page = await this.getPage(pageId);
      const content = page.body?.storage?.value || "";
      const parsed = options?.debug ? this.collectParsedTablesWithDebug(content) : { tables: this.collectParsedTables(content) };
      const entries = parsed.tables.map((table) => ({
        ...table.entry,
        section: table.section || "",
        sourceTableIndex: table.qaIndex,
        sourceTableHtml: table.html,
        isDirty: false,
      }));

      const viewContent = page.body?.view?.value || "";
      if (viewContent && entries.length > 0) {
        this.enrichEntriesFromView(entries, viewContent);
      }

      const jiraServerId = await this.resolveJiraServerId(content);

      try {
        const attachments = await this.rawClient.getAttachments(pageId);
        for (const entry of entries) {
          for (const img of entry.images) {
            const normalizedImgName = normalizeFilename(img.name);
            const matchedAtt = attachments.find((att: any) => {
              const normalizedAttTitle = normalizeFilename(att.title);
              return normalizedAttTitle === normalizedImgName;
            });

            if (matchedAtt && matchedAtt._links?.download) {
              try {
                const buffer = await this.rawClient.downloadAttachment(matchedAtt._links.download);
                const base64 = buffer.toString("base64");
                const mimeType = matchedAtt.metadata?.mediaType || matchedAtt.mediaType || "image/png";
                img.data = `data:${mimeType};base64,${base64}`;
              } catch (downloadErr) {
                logger.error("Confluence", `Failed to download attachment ${img.name}:`, downloadErr);
              }
            }
          }
        }
      } catch (attErr) {
        logger.error("Confluence", "Failed to fetch attachments list for page:", attErr);
      }

      return {
        pageId,
        pageTitle: page.title || "",
        contentLoaded: true,
        entries,
        jiraServerId,
        debug: parsed.debug
          ? {
              ...parsed.debug,
              pageId,
              pageTitle: page.title || "",
            }
          : undefined,
      };
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      logger.error("Confluence", `Failed to fetch page content for ${pageId}:`, error);
      return {
        pageId,
        pageTitle: "",
        contentLoaded: false,
        entries: [],
        error: message,
        debug: options?.debug
          ? {
              pageId,
              pageTitle: "",
              rawPageContent: "",
              contentLength: 0,
              tableCount: 0,
              parsedTableCount: 0,
              skippedTableCount: 0,
              unmatchedHtmlChunks: [],
              tables: [],
            }
          : undefined,
      };
    }
  }

  private parseSingleTable(tableHtml: string, debug = false): { entry: any; debug?: ParseConfluenceParseDebugTable } {
    const entry: any = {
      id: "",
      testCaseNo: "",
      functionName: "",
      scenario: "",
      category: "Positive",
      inputData: "",
      inputDataFormat: "plain",
      steps: "",
      stepsFormat: "plain",
      expectedResult: "",
      expectedResultFormat: "plain",
      result: "PASS",
      images: [] as { id: string; name: string; data: string; order: number; note?: string; group?: string }[],
    };
    const debugTable: ParseConfluenceParseDebugTable | undefined = debug
      ? {
          index: 0,
          parsed: false,
          rawHtml: tableHtml,
          rows: [],
          mappedFields: [],
        }
      : undefined;
    const recordRow = (row: ParseConfluenceParseDebugRow) => {
      debugTable?.rows.push(row);
    };

    const rowRegex = /<tr\b[^>]*>[\s\S]*?<\/tr>/gi;
    let rowMatch;

    while ((rowMatch = rowRegex.exec(tableHtml)) !== null) {
      const rowHtml = rowMatch[0];
      const labelMatch = rowHtml.match(/<td[^>]*>[\s\S]*?<strong>(.*?)<\/strong>[\s\S]*?<\/td>/i);
      if (!labelMatch) {
        recordRow({
          label: null,
          status: "missing-label",
          reason: "Row tidak memiliki label <strong> yang bisa dipetakan",
          rawHtml: rowHtml,
        });
        continue;
      }

      const label = labelMatch[1].trim();

      const tdRegex = /<td[^>]*>([\s\S]*?)<\/td>/gi;
      const tds: string[] = [];
      let tdMatch;
      while ((tdMatch = tdRegex.exec(rowHtml)) !== null) {
        tds.push(tdMatch[1].trim());
      }
      if (tds.length < 2) {
        recordRow({
          label,
          status: "empty-value",
          reason: "Row memiliki label, tetapi value cell tidak lengkap",
          rawHtml: rowHtml,
        });
        continue;
      }
      const value = tds[1];
      const valuePreview = stripHtml(this.cleanConfluenceMacros(value)).replace(/\s+/g, " ").trim().slice(0, 240);

      switch (label) {
        case "No. Test Case":
          entry.testCaseNo = this.extractTextValue(value);
          debugTable?.mappedFields.push("testCaseNo");
          recordRow({ label, status: "mapped", mappedField: "testCaseNo", rawHtml: rowHtml, valuePreview });
          break;
        case "Function":
          entry.functionName = this.extractTextValue(value);
          debugTable?.mappedFields.push("functionName");
          recordRow({ label, status: "mapped", mappedField: "functionName", rawHtml: rowHtml, valuePreview });
          break;
        case "Scenario":
          entry.scenario = this.extractTextValue(value);
          debugTable?.mappedFields.push("scenario");
          recordRow({ label, status: "mapped", mappedField: "scenario", rawHtml: rowHtml, valuePreview });
          break;
        case "Kategori":
          entry.category = this.extractTextValue(value);
          debugTable?.mappedFields.push("category");
          recordRow({ label, status: "mapped", mappedField: "category", rawHtml: rowHtml, valuePreview });
          break;
        case "Input Data":
          entry.inputData = this.extractTextLines(value);
          entry.inputDataFormat = detectListFormat(value);
          debugTable?.mappedFields.push("inputData");
          recordRow({ label, status: "mapped", mappedField: "inputData", rawHtml: rowHtml, valuePreview });
          break;
        case "Steps":
          entry.steps = this.extractTextLines(value);
          entry.stepsFormat = detectListFormat(value);
          debugTable?.mappedFields.push("steps");
          recordRow({ label, status: "mapped", mappedField: "steps", rawHtml: rowHtml, valuePreview });
          break;
        case "Expected Result":
          entry.expectedResult = this.extractTextLines(value);
          entry.expectedResultFormat = detectListFormat(value);
          debugTable?.mappedFields.push("expectedResult");
          recordRow({ label, status: "mapped", mappedField: "expectedResult", rawHtml: rowHtml, valuePreview });
          break;
        case "Result":
          entry.result = this.extractResult(value);
          debugTable?.mappedFields.push("result");
          recordRow({ label, status: "mapped", mappedField: "result", rawHtml: rowHtml, valuePreview });
          break;
        case "Screen Capture": {
          const orderedAttachmentBlocks = value.match(/<li\b[^>]*data-qa-attachment-order=["'][^"']+["'][^>]*>[\s\S]*?<\/li>/gi) || [];
          if (orderedAttachmentBlocks.length > 0) {
            let attachmentIndex = 0;
            for (const block of orderedAttachmentBlocks) {
              const orderMatch = block.match(/data-qa-attachment-order=["'](\d+)["']/i);
              const noteMatch = block.match(/data-qa-attachment-note=["']([^"']*)["']/i);
              const groupMatch = block.match(/data-qa-attachment-group=["']([^"']*)["']/i);
              const attachmentNames = extractAttachmentRefs(block).map((ref) => ref.name);
              for (const name of attachmentNames) {
                entry.images.push({
                  id: `${name}::${attachmentIndex}`,
                  name,
                  data: "",
                  order: orderMatch ? Number(orderMatch[1]) : attachmentIndex + 1,
                  note: noteMatch ? decodeHtmlEntities(noteMatch[1]) : "",
                  group: groupMatch ? decodeHtmlEntities(groupMatch[1]) : "",
                });
                attachmentIndex++;
              }
            }
            debugTable?.mappedFields.push("images");
            recordRow({
              label,
              status: "mapped",
              mappedField: "images",
              rawHtml: rowHtml,
              valuePreview,
            });
            break;
          }

          const blockRegex = /<li\b[^>]*>[\s\S]*?<\/li>|<p\b[^>]*>[\s\S]*?<\/p>/gi;
          let attachmentIndex = 0;
          let pendingNote = "";
          let blockMatch;
          let blockCount = 0;
          while ((blockMatch = blockRegex.exec(value)) !== null) {
            blockCount++;
            const block = blockMatch[0];
            const attachmentRefs = extractAttachmentRefs(block);
            if (attachmentRefs.length > 0) {
              const group = latestAttachmentGroupBefore(value, blockMatch.index);
              for (let refIndex = 0; refIndex < attachmentRefs.length; refIndex++) {
                const ref = attachmentRefs[refIndex];
                const segmentStart = refIndex === 0 ? 0 : attachmentRefs[refIndex - 1].end;
                const segment = block.slice(segmentStart, ref.index);
                const inlineNote = this.extractTextValue(stripAttachmentMarkup(segment)).trim();
                const note = inlineNote && inlineNote !== "-"
                  ? inlineNote
                  : refIndex === 0
                    ? pendingNote
                    : "";
                attachmentIndex++;
                entry.images.push({
                  id: `${ref.name}::${attachmentIndex}`,
                  name: ref.name,
                  data: "",
                  order: attachmentIndex,
                  note,
                  group,
                });
              }
              pendingNote = "";
              continue;
            }

            const text = this.extractTextValue(block).trim();
            if (text && text !== "-" && text !== "Click here to expand...") {
              pendingNote = pendingNote ? `${pendingNote}\n${text}` : text;
            }
          }
          debugTable?.mappedFields.push("images");
          recordRow({
            label,
            status: "mapped",
            mappedField: "images",
            rawHtml: rowHtml,
            reason: blockCount === 0 ? "Screen Capture kosong" : "Screen Capture dipetakan tanpa marker attachment eksplisit",
            valuePreview,
          });
          break;
        }
        default:
          recordRow({
            label,
            status: "unmapped-label",
            reason: "Label tidak dikenali oleh parser",
            rawHtml: rowHtml,
            valuePreview,
          });
          break;
      }
    }

    if (debugTable) {
      debugTable.parsed = Boolean(entry.testCaseNo);
      debugTable.reason = entry.testCaseNo ? undefined : "No. Test Case tidak ditemukan";
      debugTable.entrySummary = {
        testCaseNo: entry.testCaseNo,
        functionName: entry.functionName,
        scenario: entry.scenario,
        category: entry.category,
        result: entry.result,
        imageCount: entry.images.length,
      };
    }

    return { entry, debug: debugTable };
  }

  private collectParsedTablesWithDebug(content: string): {
    tables: Array<{ qaIndex: number; html: string; start: number; end: number; entry: any; section: string }>;
    debug?: ParseConfluenceParseDebugReport;
  } {
    const tables: Array<{ qaIndex: number; html: string; start: number; end: number; entry: any; section: string }> = [];
    const debugTables: ParseConfluenceParseDebugTable[] = [];
    const unmatchedHtmlChunks: ParseConfluenceParseDebugChunk[] = [];
    let currentSection = "";
    const regex = /<h1\b[^>]*>|<table\b[^>]*>/gi;
    let match: RegExpExecArray | null;
    let qaIndex = 0;
    let tableIndex = 0;
    let lastEnd = 0;

    while ((match = regex.exec(content)) !== null) {
      const gap = content.slice(lastEnd, match.index).trim();
      if (gap) {
        unmatchedHtmlChunks.push({
          index: unmatchedHtmlChunks.length,
          reason: "HTML di luar tabel tidak diproses parser",
          rawHtml: gap,
        });
      }

      const chunk = match[0];
      if (chunk.startsWith("<h1")) {
        const endMatch = content.slice(regex.lastIndex).match(/<\/h1\s*>/i);
        if (!endMatch || endMatch.index === undefined) {
          regex.lastIndex = match.index + chunk.length;
          continue;
        }
        const start = match.index;
        const end = regex.lastIndex + endMatch.index + endMatch[0].length;
        const fullChunk = content.slice(start, end);
        regex.lastIndex = end;
        const text = fullChunk.replace(/<[^>]+>/g, "").trim();
        if (text) currentSection = text;
      } else {
        const tableBlock = this.extractBalancedTagBlock(content, match.index, "table");
        if (!tableBlock) {
          regex.lastIndex = match.index + chunk.length;
          continue;
        }
        regex.lastIndex = tableBlock.end;
        const parsed = this.parseSingleTable(tableBlock.html, true);
        const entry = parsed.entry;
        const debugTable = parsed.debug;
        const isParsed = Boolean(entry && entry.testCaseNo);

        if (debugTable) {
          debugTable.index = tableIndex;
          debugTable.parsed = isParsed;
          debugTable.reason = isParsed ? undefined : "No. Test Case tidak ditemukan";
          debugTables.push(debugTable);
        }

        if (isParsed) {
          tables.push({
            qaIndex,
            html: tableBlock.html,
            start: match.index,
            end: tableBlock.end,
            entry,
            section: currentSection,
          });
          qaIndex++;
        }
      }

      lastEnd = regex.lastIndex;
      tableIndex++;
    }

    const tail = content.slice(lastEnd).trim();
    if (tail) {
      unmatchedHtmlChunks.push({
        index: unmatchedHtmlChunks.length,
        reason: "HTML setelah tabel terakhir tidak diproses parser",
        rawHtml: tail,
      });
    }

    if (debugTables.length === 0 && unmatchedHtmlChunks.length === 0) {
      return { tables };
    }

    return {
      tables,
      debug: {
        pageId: "",
        rawPageContent: content,
        contentLength: content.length,
        tableCount: debugTables.length,
        parsedTableCount: tables.length,
        skippedTableCount: debugTables.filter((table) => !table.parsed).length,
        unmatchedHtmlChunks,
        tables: debugTables,
      },
    };
  }

  private enrichEntriesFromView(entries: any[], viewContent: string): void {
    const tableRegex = /<table[^>]*>[\s\S]*?<\/table>/gi;
    const viewTables: string[] = [];
    let tableMatch;
    while ((tableMatch = tableRegex.exec(viewContent)) !== null) {
      viewTables.push(tableMatch[0]);
    }

    let viewIndex = 0;
    for (const entry of entries) {
      while (viewIndex < viewTables.length) {
        const enriched = this.tryEnrichEntryScenarioFromViewTable(entry, viewTables[viewIndex]);
        viewIndex++;
        if (enriched) break;
      }
    }
  }

  private tryEnrichEntryScenarioFromViewTable(entry: any, viewTableHtml: string): boolean {
    const rowRegex = /<tr\b[^>]*>[\s\S]*?<\/tr>/gi;
    let rowMatch;

    while ((rowMatch = rowRegex.exec(viewTableHtml)) !== null) {
      const rowHtml = rowMatch[0];
      const labelMatch = rowHtml.match(/<td[^>]*>[\s\S]*?<strong>(.*?)<\/strong>[\s\S]*?<\/td>/i);
      if (!labelMatch) continue;

      if (labelMatch[1].trim() !== "Scenario") continue;

      const tdRegex = /<td[^>]*>([\s\S]*?)<\/td>/gi;
      const tds: string[] = [];
      let tdMatch;
      while ((tdMatch = tdRegex.exec(rowHtml)) !== null) {
        tds.push(tdMatch[1].trim());
      }

      if (tds.length >= 2) {
        const cellHtml = tds[1];
        const jiraKeyMatch = cellHtml.match(/data-jira-key=["']([^"']+)["']/i);
        const issueKey = jiraKeyMatch?.[1]?.trim();

        if (issueKey) {
          const summaryMatch = cellHtml.match(
            /<span\s+class\s*=\s*["'][^"']*\bsummary\b[^"']*["'][^>]*>([\s\S]*?)<\/span>/i
          );
          const summary = summaryMatch ? stripHtml(summaryMatch[1]).trim() : "";

          if (summary && !/Getting issue details/i.test(summary)) {
            entry.scenario = summary;
          } else {
            entry.scenario = issueKey;
          }
          entry.scenarioIssueKey = issueKey;
          entry.scenarioSummary = summary && !/Getting issue details/i.test(summary) ? summary : "";
          return true;
        }
      }

      break;
    }

    return false;
  }

  private cleanJiraMacro(macroHtml: string): string {
    const params: string[] = [];
    const paramRegex = /<ac:parameter ac:name=["']([^"']+)["']>([\s\S]*?)<\/ac:parameter>/gi;
    let match;
    while ((match = paramRegex.exec(macroHtml)) !== null) {
      const name = match[1].toLowerCase();
      if (name === "key" || name === "issuekey") {
        let value = match[2].trim();
        const riValue = value.match(/ri:value=["']([^"']+)["']/i);
        if (riValue) value = riValue[1];
        params.push(value);
      }
    }
    if (params.length > 0) return params.join(", ");
    const keyFallback = macroHtml.match(/([A-Z]{2,}-\d+)/);
    return keyFallback ? keyFallback[1] : "";
  }

  private cleanConfluenceMacros(html: string): string {
    let result = html;

    result = result.replace(
      /<ac:structured-macro[\s\S]*?ac:name=["']jira[\s\S]*?<\/ac:structured-macro>/gi,
      (match) => this.cleanJiraMacro(match)
    );

    result = result.replace(
      /<span[^>]*data-jira-key=["']([^"']+)["'][^>]*>[\s\S]*?<\/span>/gi,
      "$1"
    );

    result = result.replace(
      /<span[^>]*class=["'][^"']*\bjira-issue\b[^"']*["'][^>]*>[\s\S]*?<\/span>/gi,
      (match) => {
        const keyMatch = match.match(/([A-Z]{2,}-\d{1,9})/);
        return keyMatch ? keyMatch[1] : "";
      }
    );

    result = result.replace(
      /<ac:plain-text-link-body>\s*(?:<!\[CDATA\[)?([^\]<]+)(?:\]\]>)?\s*<\/ac:plain-text-link-body>/gi,
      "$1"
    );

    result = result.replace(/-\s*Getting issue details\.\.\./gi, "");
    result = result.replace(/\bGetting issue details\.\.\.\s*/gi, "");

    return result;
  }

  private extractTextValue(value: string): string {
    return stripHtml(this.cleanConfluenceMacros(value)).trim();
  }

  private extractTextLines(value: string): string {
    const cleaned = this.cleanConfluenceMacros(value);
    const liRegex = /<li\b[^>]*>([\s\S]*?)<\/li>/gi;
    const liItems: string[] = [];
    let match;
    while ((match = liRegex.exec(cleaned)) !== null) {
      liItems.push(stripHtml(match[1]).trim());
    }
    if (liItems.length > 0) {
      return liItems.join("\n");
    }

    const pRegex = /<p[^>]*>([\s\S]*?)<\/p>/gi;
    const pItems: string[] = [];
    while ((match = pRegex.exec(cleaned)) !== null) {
      const text = stripHtml(match[1]).trim();
      if (text) {
        pItems.push(text);
      }
    }
    if (pItems.length > 0) {
      return pItems.join("\n");
    }

    const brItems = cleaned.split(/<br\s*\/?>/i).map((s) => stripHtml(s).trim()).filter(Boolean);
    if (brItems.length > 1) {
      return brItems.join("\n");
    }

    return this.extractTextValue(cleaned);
  }

  private extractResult(value: string): string {
    const text = stripHtml(value).toLowerCase();

    const passPos = text.indexOf("pass");
    const failPos = text.indexOf("fail");
    const passedPos = text.indexOf("passed");
    const failedPos = text.indexOf("failed");

    if (passedPos !== -1 && failedPos !== -1) {
      return passedPos < failedPos ? "PASS" : "FAILED";
    }
    if (passPos !== -1 && failPos !== -1) {
      return passPos < failPos ? "PASS" : "FAILED";
    }
    if (passedPos !== -1 || (passPos !== -1 && failPos === -1)) {
      return "PASS";
    }
    if (failedPos !== -1 || failPos !== -1) {
      return "FAILED";
    }

    return "PASS";
  }
}
